DRUPAL_PHPUNIT_ENV = SIMPLETEST_BASE_URL=http://127.0.0.1:8080 SIMPLETEST_DB=pgsql://drupal:drupal@postgres/drupal BROWSERTEST_OUTPUT_BASE_URL=http://127.0.0.1:8080
DOCKER_COMPOSE_EXEC ?= docker compose exec -T

.PHONY: test test-symbol-engine test-drupal prepare-drupal-test check-sensitive-files check-repository-hygiene check-env-example-policy check-node-dependency-policy check-docker-image-policy check-github-actions-policy audit-drupal-dependencies build-symbol-engine-production smoke-symbol-engine-production drush-updb drush-cr
...
check-env-example-policy:
	./scripts/check-env-example-policy.js

check-node-dependency-policy:
	./scripts/check-node-dependency-policy.js

check-docker-image-policy:
	./scripts/check-docker-image-policy.js

check-github-actions-policy:
	./scripts/check-github-actions-policy.js
...
test-drupal: prepare-drupal-test
	$(DOCKER_COMPOSE_EXEC) drupal sh -lc 'cd /opt/drupal && env $(DRUPAL_PHPUNIT_ENV) vendor/bin/phpunit -c phpunit.xml.dist --group symbol_atomic_swap'
...
drush-updb:
	$(DOCKER_COMPOSE_EXEC) drupal sh -lc 'cd /opt/drupal && vendor/bin/drush updatedb -y'
