...
final class SwapOfferRoutesTest extends BrowserTestBase {
  ...
  public function testAcceptOfferResolvesTakerPublicKeyFromAddress(): void {
    ...
    $id = $repository->insert($this->offerValues([
      ...
      'leg1_amount' => '100',
      'leg2_recipient_address' => 'TAEF3VF4OYCKPSSJQAAN4FS2WAZLC6IKKCE3UIQ',
  ...
  public function testOfferOperationRoutesRenderForOperators(): void {
    ...
    $assert_session->pageTextContains('Paste the root signed payload returned by the external signing app.');
    $assert_session->buttonExists('Verify root signed payload');
    $this->drupalGet('/symbol-atomic-swap/settlements/' . $id . '/submit-aggregate-signer-json');
    $assert_session->statusCodeEquals(200);
    $assert_session->fieldExists('Aggregate signer JSON');
